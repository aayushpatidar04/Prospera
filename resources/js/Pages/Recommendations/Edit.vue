...
const form = useForm({
    ...
    exchange: data.recommendation?.exchange || '',
    recommendation_type: data.recommendation?.recommendation_type || 'buy',
    ...
    duration: data.recommendation?.duration || 'intraday',
    risk_level: data.recommendation?.risk_level || 'low',
    analyst_notes: data.recommendation?.analyst_notes || ''
})
...
const submitForm = () => {
    form.patch(route('recommendations.edit', data.recommendation.id), {
        preserveScroll: true,
        onSuccess: () => {
